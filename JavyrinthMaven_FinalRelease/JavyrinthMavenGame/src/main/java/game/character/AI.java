...
public class AI {
	...
	GamePanel gamePanel;
	/**
	 *Coordinates of a station point in path
	 */
	class Station {
		int x, y;
	}
	...
	/**
	 * This would be used if we choose different modes for Minotaur's AI
	 * We use the HIDE role for our game
	 */
	 ...
	 * @return one route
	 * 
	 * Ousiastika periexetai to prwto(a) simeio(a) apo epilogi apo tis prwtes kiniseis poy epistrefetai 
	 * apo tin evaluateRoute(praktika epilegetai mia kinini),
	 * wste na dwthei ston character1 ena yposynolo kinisewn na ektelesei.
	 * Kathorizetai i symperifora tou "xaraktira" tis AI
	 ...
	 * H evaluateNextRoute ylopoiei tin texniki pleyra tis AI.
	 */
	 ...
	 * H epilogi ayti vasizetai stin paroysa thesi toy character1(to robot) 
	 * kai toy character2,
	...
	private Route evaluateRoute(int c1x, int c1y, DIR prevDir, int c2x, int c2y,
		...
		/** 
		 * ArrayList routeDistancesAvg
		 *  Mesos oros apostasewn kathe simeiou gia ena route tou character1 
		 *  apo kathe simeio gia ena route tou character2
		 */
		ArrayList<Integer> routeDistancesAvg = new ArrayList<Integer>();
		
		/**
		 *ArrayList routeDistancesAvgSum
		 *  Athroisma meswn orwn apostasewn gia kathe route tou character1 
		 *  apo kathe route tou character2
		 *  Me skopo na epileksw route megistopoiontas/elaxistopoiontas 
		 *  ayto to athroisma
		 */
		...
		int index;
		/**
		 * Then it returns the index of first occurance of min value
		 */
		switch (role) {
		case CHASE:
		...
		/**
		 *  Removes home station so you have only next moves directions and stations,
	 ...
	 * @param y
	 * @param prevDir 
	 * @param maxNumOfRouteStations
	 * @return ArrayList apo antikeimena Route,
	 * lamvanontas ypopsin tin parousa thesi toy character 
	 * kai tin proigoumeni toy kinisi,
	 * apofeygontas etsi ta mpros pisw kata tin dimioyrgia tou kathe route
	 * ta opoia telika tha exoun ena megisto arithmo station maxNumOfRouteStations.
	...
	public ArrayList<Route> getRoutes(int x, int y, DIR prevDir, int maxNumOfRouteStations) {
		...
		do {	
			...
			for (int r = newRoutesAddedStart; r <= newRoutesAddedEnd; r++) {
				...
				/**
				 * Gia na min exw exception sta dead ends me ta avoid back-n-forth
				 * kai epistrefei null routes
				 */
		 ...
		 */
		/**
		 *	Take a sublist of Routes with the latest added stations
		 *	(Routes with same size = maxNumOfRouteStations)
		 *	If you want all sizes
		 *	validPaths for some reason it causes Exception in evaluateRoute	
		 */
		validPaths =
	 ...
	 * @param x
	 * @param y
	 * @return ArrayList with the sides of tile that collide (the adjacent tiles are solid)
